...
# def get_pwcca_sims_cross_validation(lg_feat_dict, sp_feat_dict, lg_nn_features, sp_nn_features, n_folds=5, shuffle=False, variance_ratio=0.99):
#     """
#         lg_feat_dict: key: nn_feat_name in lg_nn_features, val: lg_feat (n_samples * n_lg_features)
#         sp_feat_dict: key: nn_feat_name in sp_nn_features, val: sp_feat (n_samples * n_sp_features)
#     """
#     pwcca_mat_folds, pwcca_mat_folds_valid = [], []
#     kf = model_selection.KFold(n_splits=n_folds, shuffle=shuffle)

#     for k1 in lg_nn_features:
#         tmp, tmp2 = [], []
#         lg_feat = lg_feat_dict[k1]
#         lg_feat_pca = scale_and_pca_one_dataset(lg_feat, variance_ratio=variance_ratio)
#         # lg_feat_pca = lg_feat
#         for k2 in sp_nn_features:
#             sp_feat = sp_feat_dict[k2]  
#             sp_feat_pca = scale_and_pca_one_dataset(sp_feat, variance_ratio=variance_ratio)
#             # sp_feat_pca = sp_feat
#             print("lg shape: {}, sp shape: {}".format(lg_feat_pca.shape, sp_feat_pca.shape))
#             cur_sims, cur_sims_valid = [], []
#             for i, (train, test) in enumerate(kf.split(lg_feat_pca)):
#                 valid_len = len(train) // 5
#                 valid = train[-1* valid_len:]
#                 train = train[0 : len(train) - valid_len]
#                 # lg_feat_train, lg_feat_test = lg_feat_pca[train, :].T, lg_feat_pca[test, :].T
#                 lg_feat_train, lg_feat_test, lg_feat_valid = lg_feat_pca[train, :].T, lg_feat_pca[test, :].T, lg_feat_pca[valid, :].T
#                 # sp_feat_train, sp_feat_test = sp_feat_pca[train, :].T, sp_feat_pca[test, :].T
#                 sp_feat_train, sp_feat_test, sp_feat_valid = sp_feat_pca[train, :].T, sp_feat_pca[test, :].T, sp_feat_pca[valid, :].T
#                 # pwcca_sim, _, _ = pwcca.compute_pwcca_on_test_set(lg_feat_train, sp_feat_train, lg_feat_test, sp_feat_test, epsilon=1e-10)
#                 pwcca_sim_valid, _, _ = pwcca.compute_pwcca_on_test_set(lg_feat_train, sp_feat_train, lg_feat_valid, sp_feat_valid, epsilon=1e-10)
#                 pwcca_sim, _, _ = pwcca.compute_pwcca_on_test_set(lg_feat_train, sp_feat_train, lg_feat_test, sp_feat_test, epsilon=1e-10)
#                 cur_sims_valid.append(pwcca_sim_valid)
#                 cur_sims.append(pwcca_sim)
#             print("validation： {}, {}, {}".format(k1, k2, cur_sims_valid))
#             print("{}, {}, {}".format(k1, k2, cur_sims))
#             tmp.append(cur_sims)
#             tmp2.append(cur_sims_valid)
#         pwcca_mat_folds.append(tmp)  
#         pwcca_mat_folds_valid.append(tmp2)
#     mean_pwcca_sims = np.mean(pwcca_mat_folds, axis=2)
#     mean_pwcca_sims_valid = np.mean(pwcca_mat_folds_valid, axis=2)
#     print("Done")    
#     return pwcca_mat_folds, mean_pwcca_sims, mean_pwcca_sims_valid
...
#     mean_pwcca_sims = np.mean(pwcca_mat_folds, axis=2)
#     print("Done")    
#     return pwcca_mat_folds, mean_pwcca_sims
